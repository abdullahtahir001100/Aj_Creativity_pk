...
const Cart = () => {
  ...
  // Helper: generate unique key for item
  const getKey = (item) => `${item.id}_${item.size || "NA"}_${item.color || "NA"}`;

  const handleQtyChange = (itemKey, value) => {
    const newCart = cartItems.map((item) => {
      if (getKey(item) === itemKey) {
  ...
  const removeItem = (itemKey) => {
    const newCart = cartItems.filter((item) => getKey(item) !== itemKey);
    setCartItems(newCart);
    localStorage.setItem("cart", JSON.stringify(newCart));
    window.dispatchEvent(new Event("cartUpdated"));
  };
  ...
  const applyCoupon = (code) => {
    ...
    if (code.toUpperCase() === "SAVE500" && !couponApplied) {
      ...
      localStorage.setItem("couponDiscount", JSON.stringify(discountValue)); // FIXED: Save discount to localStorage
          ...
          >
            ...
            {cartItems.length === 0 && (
              <div style={{ color: "red", marginTop: 8, fontSize: 14 }}>
                Please add at least one product to your cart before proceeding to
                checkout.
              </div>
            )}
          </div>
